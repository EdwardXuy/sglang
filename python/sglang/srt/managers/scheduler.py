...
class TestEnableMixedChunk(CustomTestCase):
    ...
    def _benchmark_mixed_load(self):
        ...
        for _ in range(50):
            t = threading.Thread(target=self._send_single_request, args=(self.LONG_PROMPT, 32))
            threads.append(t)

        # Launch 50 short text requests
        for _ in range(50):
            t = threading.Thread(target=self._send_single_request, args=(self.SHORT_PROMPT, 32))
            threads.append(t)
