...
class TestVLMModels(CustomTestCase):
    ...
    def _run_vlm_mmmu_test(
        ...
        Args:
            ...
            capture_output: Whether to capture server stdout/stderr
        """
        warnings.filterwarnings("ignore", category=FutureWarning, module="unclosed.*socket")
        print(f"\nTesting model: {self.model}{test_name}")
        ...
        try:
            ...
            self.assertGreaterEqual(
                metrics["score"],
                self.mmmu_accuracy,
                f"Model {self.model} accuracy ({metrics['score']:.4f}) below expected threshold ({self.mmmu_accuracy:.4f}){test_name}",
            )
