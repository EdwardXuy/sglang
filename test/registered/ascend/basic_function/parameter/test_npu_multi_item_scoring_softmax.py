...
class TestMultiItemScoringItemFirst(CustomTestCase):
    """Test item_first parameter functional behavior in single-item mode (no delimiter).

    A second server is started without --multi-item-scoring-delimiter so that
    item_first actively affects prompt construction order (item + query vs query + item).

    [Test Category] Feature
    [Test Target] item_first parameter; /v1/score (single-item mode)
    """
    ...
    def setUpClass(cls):
        ...
        logger.info("Single-item mode server started (no delimiter).")

    @classmethod
    def tearDownClass(cls):
        kill_process_tree(cls.process.pid)
        logger.info("Single-item mode server terminated.")

    def test_item_first_true_semantic_correctness(self):
        """item_first=True: semantic ordering is preserved with reversed prompt construction.

        Checks:
          - HTTP 200, correct response structure.
          - Correct item ("It is 3"): Yes-prob > No-prob.
          - Wrong items ("It is 4", "It is 5"): Yes-prob < No-prob.
        """
        response = send_score_request(
        ...
        self.assertLess(scores[2][0], scores[2][1],
                        "Wrong item 'It is 5': Yes-prob should be less than No-prob.")
        logger.info("item_first=True semantic ordering verified in single-item mode.")

    def test_item_first_changes_scores_in_single_item_mode(self):
        """item_first=True and False produce different scores in single-item mode.

        Unlike multi-item delimiter mode (where item_first is ignored),
        in single-item mode the parameter actively changes prompt token order,
        so scores must differ between the two settings.
        """
        resp_false = send_score_request(
            base_url=self.base_url,
            query=_QUERY,
            items=_ITEMS,
            label_token_ids=_LABEL_TOKEN_IDS,
        ...
        self.assertNotEqual(
            scores_false,
            scores_true,
            "item_first=True and False should produce different scores in single-item mode "
            "(parameter is not ignored when delimiter is inactive).",
        )
        logger.info("item_first=True vs False score distinction verified in single-item mode.")
