"""
Testcase: Verify basic multi-item scoring functionality with --multi-item-scoring-delimiter.

This file covers:
- Case 1 (Basic): response structure, output shape, float type, semantic score correctness.
- Case 2 (Delimiter Token validation, merged): verifying delimiter positions produce correct
  score extraction, implicitly validated by the semantic correctness assertions.
- Case 5 (Text input, merged): query and items are provided as raw text strings; the server
  handles tokenization internally.

Background on multi-item scoring data flow:
  Combined sequence: query <delimiter> item1 <delimiter> item2 <delimiter> item3 <delimiter>
  The model performs a single forward pass. Logprobs are extracted at each delimiter position.
  The first delimiter (between query and item1) is skipped; remaining positions yield item scores.

  This means score for item_i reflects the model distribution GIVEN all tokens up to and
  including item_i (items are processed sequentially, earlier items provide context).

[Test Category] Parameter
[Test Target] --multi-item-scoring-delimiter; --disable-radix-cache; --chunked-prefill-size
...
class TestMultiItemScoringBasic(CustomTestCase):
    ...
    def test_structure_and_semantic_correctness_with_softmax(self):
        ...
        # Step 2: 'scores' field presence and outer list length
        ...
        # Step 3+4+5: per-item sub-list validation
        ...
        # Step 6: Semantic correctness
        # "It is 3" is correct (1+2=3): model should assign higher probability to Yes token
        self.assertGreater(
            scores[0][0],
            scores[0][1],
        ...
        # "It is 4" is incorrect: model should assign higher probability to No token
        self.assertLess(
            scores[1][0],
            scores[1][1],
        ...
        # "It is 5" is incorrect: model should assign higher probability to No token
        self.assertLess(
            scores[2][0],
            scores[2][1],
