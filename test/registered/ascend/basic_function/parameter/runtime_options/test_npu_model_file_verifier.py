...
class TestModelFileVerifier(_FakeModelTestCase):
    # Test 1: Detect bit rot (simulate file corruption)
    def test_detect_bit_rot(self):
        # Generate checksum file (records SHA256 and size for all files)
    ...
    def test_detect_missing_file(self):
        ...
        # Expect IntegrityError with config.json in error message
        with self.assertRaises(IntegrityError) as ctx:
            verify(model_path=self.test_dir, checksums_source=checksums_file)
        self.assertIn("config.json", str(ctx.exception))

    # Test 3: Verify SHA256 calculation accuracy
    def test_compute_sha256(self):
        # Create test file with content "hello world"
        test_file = os.path.join(self.test_dir, "test.bin")
        content = b"hello world"
        with open(test_file, "wb") as f:
            f.write(content)

        # Calculate SHA256 (utility function vs native hashlib)
    ...
    # Test 4: Verify parallel checksum computation
    def test_parallel_checksum_computation(self):
        # Create 10 shard files (simulate large model sharded weights)
        for i in range(10):
            _create_test_file(
                self.test_dir, f"shard_{i}.safetensors", f"content_{i}".encode() * 1000
            )

        # Generate checksum file with 4 worker threads
        checksums_file = os.path.join(self.test_dir, "checksums.json")
        result = generate_checksums(
            source=self.test_dir, output_path=checksums_file, max_workers=4
        )

        # At least 10 files should have checksums generated
        self.assertGreaterEqual(len(result.files), 10)

    # Test 5: Verify generated checksum file format and content match expectations
    def test_generated_json_snapshot(self):
        ...
        # Expected checksums (pre-computed fixed values)
        expected = {
            "files": {
                "config.json": {
        ...
        # Assert generated JSON matches expected values
        self.assertEqual(data, expected)

    # Test 6: Verify deprecation warning for legacy checksum file format
    def test_legacy_checksums_format_deprecated(self):
        # Create legacy format checksum file (only checksums field, no files/size)
        legacy_data = {
            "checksums": {
        ...
        # Capture warnings to verify tool recognizes legacy format and raises DeprecationWarning
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            verify(model_path=self.test_dir, checksums_source=legacy_file)
            # Assert 1 warning captured, is DeprecationWarning, and message contains "deprecated"
...
class TestModelFileVerifierCLI(_FakeModelTestCase):
    # Test 1: Generate checksum file via CLI
    def test_cli_generate(self):
        ...
        # Assert command executed successfully (return code 0), checksum file exists, and content matches expectations
    ...
    # Test 2: CLI verification succeeds for intact files
    def test_cli_verify_success(self):
        ...
        # Assert execution succeeded and output contains "verified successfully"
        self.assertEqual(result.returncode, 0, f"stderr: {result.stderr}")
        self.assertIn("verified successfully", result.stdout)

    # Test 3: CLI verification fails on corrupted files
    def test_cli_verify_fails_on_corruption(self):
        ...
        # Assert execution failed (non-zero return code) and output contains IntegrityError or "mismatch"
...
class TestModelFileVerifierWithRealModel(_RealModelTestCase):
    def _run_server_test(self, *, corrupt_weights: bool, use_hf_checksum: bool):
        ...
        if corrupt_weights:
            # Corruption scenario: output contains filename and "mismatch"
            output = stdout_io.getvalue() + stderr_io.getvalue()
            self.assertIn(corrupted_file, output)
            self.assertIn("mismatch", output.lower())
        else:
            # Normal scenario: server starts successfully and can infer normally
            try:
    ...
    def test_server_launch_with_checksum_intact(self):
        # Verify server starts normally with local checksum file and intact weights
...
def _flip_bit_in_file(file_path: str, byte_offset: int = 100, bit_position: int = 0):
    # Flip one bit in file (simulate file corruption)
