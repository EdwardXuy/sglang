"""
Test case 4.5: Verify dynamic batch tokenizer handles concurrent requests with
different sampling parameters correctly.

Internal behaviour:
  After accumulating a batch the _dynamic_batch_loop checks whether all
  collected requests share the same tokenizer kwargs.  When kwargs differ
  (different temperature, top_p, etc.) the loop falls back to processing
  each request individually instead of calling the batched tokenizer once.
  This individual-processing path must not cause errors or silently drop requests.
"""
import threading
import unittest
