...
class TestDisaggregationBase(CustomTestCase):
    """Base test class for disaggregated architecture (Prefill/Decode separation).

    Core Functions:
    - Port Planning: Allocate incremental ports for LB/Prefill/Decode services based on default test port
    - Env Adaptation: Adapt RDMA transfer backend and device parameters for CI/local environments
    - Service Management: Launch LB service, health check, and unified resource cleanup
    """
    ...
    @classmethod
    def launch_lb(cls):
        """Launch Load Balancer (LB) service for disaggregated architecture.

        Core Logic:
        - Build LB launch command and associate Prefill/Decode service addresses
        - Start process and check for launch errors
        - Wait for LB service health check to be ready
        """
        lb_command = [
            "python3",
            "-m",
    ...
    def wait_server_ready(cls, url, timeout=DEFAULT_TIMEOUT_FOR_SERVER_LAUNCH):
        """Universal method: Wait for the specified service's health check endpoint to return 200 (throw exception on timeout).

        Args:
            url: Health check endpoint URL (e.g. http://host:port/health)
            timeout: Max wait time (use test framework default timeout config by default)
        """
...
def get_rdma_devices_args():
    """Auto-match RDMA devices based on CUDA_VISIBLE_DEVICES (adapt to multi-GPU/RDMA topology in CI env).

    Core Logic:
    1. Read RDMA device list from CI environment (SGLANG_CI_RDMA_ALL_DEVICES)
    2. Map to corresponding RDMA devices based on visible GPU indices
    3. Return default device pair for exception scenarios (ensure test compatibility)
    """
